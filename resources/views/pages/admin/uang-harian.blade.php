<x-layout.app title="Data Uang Harian">
    <div id="uang-harian-root" class="flex">

        <x-layout.sidebar />

        <main class="flex-1 p-6 bg-background min-h-screen">
            <x-layout.breadcrumb :items="[['label' => 'Admin'], ['label' => 'Data Uang Harian']]" />

            <x-layout.page-header title="Data Uang Harian" description="Manajemen tarif uang harian perjalanan dinas per provinsi">
                <x-slot:actions>
            ...
            <x-layout.card class="mt-6">
                <x-slot:header>
                    <div class="flex justify-between items-center px-6 py-4 border-b border-border-custom bg-surface">
                        <h3 class="text-lg font-semibold text-text-main">Daftar Tarif Uang Harian</h3>
                    </div>
                </x-slot:header>

                <div class="p-0">
                    @php
                        $headers = ['Provinsi', 'Luar Kota', 'Dalam Kota (> 8 Jam)', 'Diklat', 'Aksi'];
                        
                        $rows = $uangHarians->map(function($item) {
                            $data = [
                                'id' => $item->id,
                            ...
                            $editUrl = route('admin.uang-harian.update', $item->id);
                            $deleteUrl = route('admin.uang-harian.destroy', $item->id);
                ...
                @if ($uangHarians->hasPages())
                    <x-slot:footer>
                        {{ $uangHarians->links('components.navigation.pagination') }}
                    </x-slot:footer>
                @endif
            </x-layout.card>
        </main>
        ...
        {{-- MODAL TAMBAH DATA --}}
        {{-- ========================================================== --}}
        <x-feedback.modal id="modal-tambah" title="Tambah Tarif Uang Harian" size="md">
            <form id="form-tambah" method="POST" action="{{ route('admin.uang-harian.store') }}" class="space-y-4">
                @csrf
                <x-form.input name="provinsi" label="Provinsi" placeholder="Contoh: Jambi" :required="true" />
                
                <div class="grid grid-cols-1 gap-4">
                    <x-form.input type="number" name="luar_kota" label="Luar Kota" :required="true" value="0" />
                    <x-form.input type="number" name="dalam_kota_lebih_8_jam" label="Dalam Kota (> 8 Jam)" :required="true" value="0" />
                    <x-form.input type="number" name="diklat" label="Diklat" :required="true" value="0" />
                </div>
            </form>
        ...
        {{-- MODAL EDIT DATA --}}
        {{-- ========================================================== --}}
        <x-feedback.modal id="modal-edit" title="Edit Tarif Uang Harian" size="md">
            <form id="form-edit" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4">
                    <x-form.input type="number" name="luar_kota" id="edit-luarkota" label="Luar Kota" :required="true" />
                    <x-form.input type="number" name="dalam_kota_lebih_8_jam" id="edit-dalamkota" label="Dalam Kota (> 8 Jam)" :required="true" />
                    <x-form.input type="number" name="diklat" id="edit-diklat" label="Diklat" :required="true" />
                </div>
            </form>
        ...
        {{-- MODAL IMPORT CSV --}}
        {{-- ========================================================== --}}
        <x-feedback.modal id="modal-import" title="Import Data Uang Harian (CSV)" size="xl">
            {{-- STEP 1: Upload --}}
                ...
                <div class="p-4 bg-blue-50 text-blue-700 text-sm rounded-md flex items-start gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <ul class="list-disc pl-4 space-y-1 text-blue-600">
                            <li>Format header kolom wajib: <code class="text-xs bg-blue-100 px-1 rounded">provinsi, luar_kota, dalam_kota_lebih_8_jam, diklat</code></li>
                        ...
                        <table class="w-full text-xs text-left border border-border-custom rounded-md overflow-hidden">
                            <thead class="bg-background/60">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-muted">Luar Kota</th>
                                    <th class="px-3 py-2 text-muted">Dalam Kota (> 8 Jam)</th>
                                    <th class="px-3 py-2 text-muted">Diklat</th>
                                </tr>
                            </thead>
        ...
        <form id="form-import-token" method="POST" action="{{ route('admin.uang-harian.import') }}" class="hidden">
            @csrf
    ...
    <script>
        function openEditModalHandler(id, url, data) {
            ...
            document.getElementById('edit-luarkota').value = data.luar_kota || 0;
            document.getElementById('edit-dalamkota').value = data.dalam_kota_lebih_8_jam || 0;
            document.getElementById('edit-diklat').value = data.diklat || 0;
        ...
        async function cekDataCSV() {
            ...
            try {
                const response = await fetch('{{ route('admin.uang-harian.validateImport') }}', {
                    method: 'POST',
                    body: formData,
                });
            ...
            if (result.valid) {
                ...
                if (result.preview && result.preview.length > 0) {
                    ...
                    result.preview.forEach(row => {
                        ...
                        tr.innerHTML = `<td class="px-3 py-2 text-text-main">${row.provinsi}</td><td class="px-3 py-2 text-muted">${row.luar_kota}</td><td class="px-3 py-2 text-muted">${row.dalam_kota_lebih_8_jam}</td><td class="px-3 py-2 text-muted">${row.diklat}</td>`;
