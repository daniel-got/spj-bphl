...
<x-layout.app title="Dashboard Admin — SPJ BPHL">
    ...
    <div class="flex min-h-screen bg-background">
        <x-layout.sidebar :items="$adminMenu">
            {{-- Footer Sidebar: Info user yang login + tombol Logout --}}
            <x-slot:footer>
                <div class="space-y-2">
                    <div class="flex items-center gap-3 px-2">
                        <x-utility.avatar :name="Auth::user()->name" size="sm" />
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-text-main truncate">{{ Auth::user()->name }}</p>
                            <p class="text-xs text-muted truncate">{{ Auth::user()->roleLabel() }}</p>
                        </div>
                    </div>
