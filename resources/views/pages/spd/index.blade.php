    ...
    <main class="grow flex flex-col px-6 py-10">
        ...
        <div class="max-w-7xl mx-auto w-full">
            ...
            <x-layout.page-header
                title="Data SPD"
                subtitle="Daftar Surat Perjalanan Dinas"
            >
                <x-slot:actions>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <x-dashboard.stat-card 
                    title="Total SPD" 
                    value="{{ $counts['all'] }}" 
                    description="Total data semua SPD" 
                    icon="document-text" 
                    color="blue" 
                    href="{{ route('spd.index') }}"
                />
                <x-dashboard.stat-card 
                    title="Disetujui" 
                    value="{{ $counts['disetujui'] }}" 
                    description="Total SPD disetujui" 
                    icon="check-circle" 
                    color="green" 
                    href="{{ route('spd.index', ['status' => 'disetujui']) }}"
                />
                <x-dashboard.stat-card 
                    title="Direvisi" 
                    value="{{ $counts['direvisi'] }}" 
                    description="Total SPD perlu direvisi" 
                    icon="exclamation-circle" 
                    color="yellow" 
                    href="{{ route('spd.index', ['status' => 'direvisi']) }}"
                />
                <x-dashboard.stat-card 
                    title="Ditolak" 
                    value="{{ $counts['ditolak'] }}" 
                    description="Total SPD ditolak" 
                    icon="x-circle" 
                    color="red" 
                    href="{{ route('spd.index', ['status' => 'ditolak']) }}"
                />
            </div>
            ...
            <div class="mb-8">
                ...
                <x-layout.card>
                    ...
                    @else
                        @php
                            $titleString = 'Riwayat SPD' . (request('status') ? ' - Status: ' . ucfirst(request('status')) : '');
                        @endphp
                        <x-slot:header>
                    ...
                    @if(request('status') && !in_array(request('status'), ['disetujui', 'ditolak', 'direvisi']))
                        ...
                        <div class="mb-4 flex items-center gap-2">
                            ...
                            <span class="inline-flex items-center {{ $activeColor }} text-xs font-semibold px-2.5 py-1 rounded-full">
                    ...
                    <div class="mb-6 mt-2 border-b border-border-custom pb-6">
                        <form method="GET" action="{{ route('spd.index') }}">
                    ...
                    @php
                        ...
                        if ($isStatusDetailView) {
                            ...
                            $rows = collect($spds ?? [])->map(function($spd, $index) {
                                $tglDiajukan = $spd->created_at ? $spd->created_at->format('d/m/Y') : ($spd->tgl_spd ? \Carbon\Carbon::parse($spd->tgl_spd)->format('d/m/Y') : '-');
                                $tglDiperiksa = $spd->updated_at ? $spd->updated_at->format('d/m/Y') : '-';
                                $catatan = $spd->status === 'disetujui' 
                                    ? '<span class="text-xs text-muted italic">Tidak butuh alasan</span>' 
                                    : e($spd->alasan ?? '-');
                                $verifikator = e($spd->nama_ppk ?? $spd->ppk ?? '-');

                                $nomorSpdLink = '<a href="' . route('spd.show', $spd->id) . '" class="text-primary hover:underline font-semibold" title="Lihat Rincian">' . e($spd->nomor_spd ?? '') . '</a>';

                                $aksiButtons = '<div class="flex items-center gap-2">
                                    <a href="' . route('spd.show', $spd->id) . '" class="inline-flex items-center bg-blue-50 hover:bg-blue-600 text-blue-700 hover:text-white text-xs font-semibold px-2.5 py-1 rounded transition-colors duration-150 border border-blue-200/50" title="Detail">
                                        Detail
                                    </a>
                                    <a href="' . route('spd.edit', $spd->id) . '" class="inline-flex items-center bg-yellow-50 hover:bg-yellow-600 text-yellow-700 hover:text-white text-xs font-semibold px-2.5 py-1 rounded transition-colors duration-150 border border-yellow-200/50" title="Edit">
                                        Edit
                                    </a>
                                </div>';
                            ...
                            $rows = collect($spds ?? [])->map(function($spd, $index) use ($showStatusAndDetails) {
                                ...
                                $tglSpd = $spd->tgl_spd instanceof \Carbon\Carbon ? $spd->tgl_spd->format('d/m/Y') : ($spd->tgl_spd ? \Carbon\Carbon::parse($spd->tgl_spd)->format('d/m/Y') : '-');
                                $tglBerangkat = $spd->tgl_berangkat instanceof \Carbon\Carbon ? $spd->tgl_berangkat->format('d/m/Y') : ($spd->tgl_berangkat ? \Carbon\Carbon::parse($spd->tgl_berangkat)->format('d/m/Y') : '-');
                                $tglKembali = $spd->tgl_kembali instanceof \Carbon\Carbon ? $spd->tgl_kembali->format('d/m/Y') : ($spd->tgl_kembali ? \Carbon\Carbon::parse($spd->tgl_kembali)->format('d/m/Y') : '-');
                                ...
                                $row = [
                                    ...
                                    '<div class="max-w-xs whitespace-normal line-clamp-2" title="' . e($spd->tujuan_kegiatan ?? '') . '">' . e($spd->tujuan_kegiatan ?? '') . '</div>',
                                ...
                                if ($showStatusAndDetails) {
                                    ...
                                    $config = $statusMap[$spd->status] ?? ['label' => ucfirst($spd->status), 'class' => 'bg-gray-100 text-gray-800'];
                                    ...
                                    if ($spd->status === 'disetujui') {
                                        ...
                                        $row[] = '<span class="text-xs font-semibold text-text-main">' . e($spd->alasan ?? '-') . '</span>';
                                    }
                                }
                    ...
                    <div class="mt-4 text-xs text-muted">
                        <span>Showing {{ count($spds ?? []) }} entries</span>
                    </div>
                </x-layout.card>
            </div>
