        ...
        <div class="max-w-5xl mx-auto w-full">
            ...
            <div class="flex items-center gap-3 mb-8">
                <a href="{{ route('spd.index') }}"
                    class="text-muted hover:text-text-main transition font-medium">
                    ← Kembali
                </a>
                ...
                <div class="bg-surface border border-border-custom rounded-xl shadow-sm p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-form.input
                            name="nomor_spd"
                            label="Nomor SPD"
                            placeholder="Contoh: 094/01/SPD/2026"
                            :value="old('nomor_spd', $spd->nomor_spd)"
                            :required="true"
                            :error="$errors->first('nomor_spd')"
                        />
                        <x-form.date-picker
                            name="tgl_spd"
                            label="Tanggal SPD"
                            :value="old('tgl_spd', $spd->tgl_spd)"
                            :required="true"
                    ...
                    <div class="border-t border-border-custom pt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <x-form.input
                                ...
                                :value="old('pegawai_ditugaskan', $spd->pegawai_ditugaskan)"
                                :required="true"
                                :error="$errors->first('pegawai_ditugaskan')"
                            />
                            <x-form.input
                                name="nip_pegawai"
                                label="NIP Pegawai"
                                placeholder="NIP"
                                :value="old('nip_pegawai', $spd->nip_pegawai)"
                                :required="true"
                                :error="$errors->first('nip_pegawai')"
                            />
                            <x-form.input
                                name="pangkat_pegawai"
                                label="Pangkat/Golongan"
                                placeholder="Pangkat/Golongan"
                                :value="old('pangkat_pegawai', $spd->pangkat_pegawai)"
                                :error="$errors->first('pangkat_pegawai')"
                            />
                            <x-form.input
                                name="jabatan_pegawai"
                                label="Jabatan"
                                placeholder="Jabatan"
                                :value="old('jabatan_pegawai', $spd->jabatan_pegawai)"
                                :error="$errors->first('jabatan_pegawai')"
                            />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-border-custom pt-6">
                        <x-form.textarea
                            ...
                            placeholder="Tuliskan tujuan kegiatan perjalanan dinas..."
                            :rows="3"
                            :value="old('tujuan_kegiatan', $spd->tujuan_kegiatan)"
                            :required="true"
                            :error="$errors->first('tujuan_kegiatan')"
                        />
                        ...
                        <div class="flex flex-col">
                            ...
                            @php
                                ...
                                if (!$destinations) {
                                    ...
                                    if (!is_array($destinations)) {
                                        $destinations = array_filter(array_map('trim', explode(',', $spd->tempat_tujuan)));
                                    }
                                }
                                ...
                                @foreach($destinations as $index => $destination)
                                    <div class="flex items-center gap-2 destination-item">
                                        ...
                                        @if(count($destinations) > 1 || $index > 0)
                                            <button type="button" class="remove-destination-btn text-danger hover:text-red-700 transition duration-150 p-2">
                            ...
                            <div class="mt-2">
                                <button type="button" id="add-destination-btn" class="inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:text-primary-hover transition duration-150">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-border-custom pt-6">
                        <x-form.date-picker
                            ...
                            :value="old('tgl_berangkat', $spd->tgl_berangkat)"
                            :required="true"
                            :error="$errors->first('tgl_berangkat')"
                        />
                        <x-form.date-picker
                            name="tgl_kembali"
                            label="Tanggal Kembali"
                            :value="old('tgl_kembali', $spd->tgl_kembali)"
                            :required="true"
                            :error="$errors->first('tgl_kembali')"
                        />
                        <x-form.input
                            name="lama_kegiatan"
                            ...
                            :value="old('lama_kegiatan', $spd->lama_kegiatan)"
                            :required="true"
                            :error="$errors->first('lama_kegiatan')"
                        />
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 border-t border-border-custom pt-6">
                        <x-form.input
                            ...
                            :value="old('kode_mak', $spd->kode_mak)"
                        ...
                        <x-form.select
                            ...
                            :selected="old('jenis_perjalanan', $spd->jenis_perjalanan)"
                            :error="$errors->first('jenis_perjalanan')"
                        ...
                        <x-form.input
                            ...
                            :value="old('berangkat_dari', $spd->berangkat_dari)"
                            :error="$errors->first('berangkat_dari')"
                        />
                        <x-form.select
                            name="alat_angkut"
                            label="Alat Angkut"
                            ...
                            :error="$errors->first('alat_angkut')"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-border-custom pt-6">
                        <x-form.select
                            ...
                            :error="$errors->first('ppk')"
                            placeholder="Pilih Pejabat Pembuat"
                        />
                        <x-form.input
                            name="nama_ppk"
                            label="Nama Pejabat PPK"
                            placeholder="Nama Pejabat"
                            :value="old('nama_ppk', $spd->nama_ppk)"
                            :error="$errors->first('nama_ppk')"
                        />
                        <x-form.input
                            name="nip_ppk"
                            label="NIP Pejabat PPK"
                            placeholder="NIP Pejabat"
                            :value="old('nip_ppk', $spd->nip_ppk)"
                            :error="$errors->first('nip_ppk')"
                        />
                    </div>
            ...
            items.forEach((item, index) => {
                ...
                if (items.length > 1) {
                    if (!removeBtn) {
                        ...
                        removeBtn.className = 'remove-destination-btn text-danger hover:text-red-700 transition duration-150 p-2';
