<x-layout.app title="Kelola SPT Pegawai - SPJ BPHL 4">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
            <main class="grow flex flex-col px-6 py-10">
                ...
                <div class="max-w-7xl mx-auto w-full">
                    ...
                    {{-- Header Halaman --}}
                    <x-layout.page-header title="Kelola SPT Pegawai" subtitle="Manajemen Data Surat Perintah Tugas (SPT) Seluruh Pegawai">
                        <x-slot:actions>
                        ...
                        <x-layout.card>
                            ...
                            @else
                                <x-slot:header>
                                    <h3 class="text-lg font-semibold text-text-main">
                                        Daftar Kelola SPT{{ request('status') ? ' — Status: ' . ucfirst(request('status')) : '' }}
                                    </h3>
                                </x-slot:header>
                            @endif
                            ...
                            @php
                                ...
                                $rows = collect($sptItems)->map(function ($spt) use (&$iteration, $statusBadgeMap) {
                                    ...
                                    // Edit link & Delete form (eksklusif di halaman ini)
                                    $canEdit = in_array($spt->status, [\App\Models\Spt::STATUS_DRAFT, \App\Models\Spt::STATUS_REVISED]);
                                    $editLink = $canEdit
