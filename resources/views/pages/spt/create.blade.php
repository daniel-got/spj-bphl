<x-layout.app title="Tambah SPT - SPJ BPHL 4">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            updateRemoveButtons();
            syncAllDropdowns();            // Auto-fill Surat Dasar (Dynamic Repeater with TomSelect)
            ...
            if (jenisTugasSelect) {
                jenisTugasSelect.addEventListener('change', function(e) {
                    ...
                    addSuratDasarRow(); // Output 1 empty row on category select
                    isInitialLoadSurat = false;
                });
            }

            // Initialize old data if any
            const oldSuratDasar = @json(old('surat_dasar', []));
            let parsedOld = [];
            ...
            if (tglBerangkat) {
                ...
                tglBerangkat.addEventListener('input', hitungHari);
            }
            if (tglKembali) {
                tglKembali.addEventListener('change', hitungHari);
                tglKembali.addEventListener('input', hitungHari);
            }
            hitungHari();
        });
    </script>
